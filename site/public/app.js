...
function shouldOpenSettings(message) {
  const settings = getSettings();
  const hasUserKey = Boolean(settings.tavilyKey || settings.geminiKey);
  return !hasUserKey && /缺少|没有通过验证|Key 无效|API Key|配置/.test(message);
}

function showTaskError(message, targetSelector) {
  const needSettings = shouldOpenSettings(message);
  const tip = needSettings
    ? "\n\n如果公共接口暂时不可用，可以在弹出的设置里填你自己的 Tavily / Gemini Key，再重试。你的 Key 只保存在当前浏览器里。"
    : "";
  const text = `${message}${tip}`;
  const target = $(targetSelector);
  if (target) target.innerHTML = escapeHtml(text).replace(/\n/g, "<br />");
  if (needSettings) openModal("settingsModal");
}
...
async function runPrompt() {
  const settings = getSettings();
  const image = await fileToDataUrl($("#promptImageInput"));
  lastPromptText = "";
  setActionNote("#promptCopyStatus", "正在分析图片，完成后可以复制提示词。");
  $("#promptReport").innerHTML = "正在看图并整理提示词...";
  const data = await postJson("/api/prompt-from-image", {
    image,
    geminiKey: settings.geminiKey,
    geminiModel: settings.geminiModel,
  });
  lastPromptText = data.content || "";
  setActionNote("#promptCopyStatus", lastPromptText ? "已生成，可以复制。" : "这次没有返回可复制内容。", !lastPromptText);
  $("#promptReport").innerHTML = renderMarkdown(lastPromptText);
}

async function copyPromptText() {
  const button = $("#copyPromptBtn");
  if (!lastPromptText.trim()) {
    setActionNote("#promptCopyStatus", "还没有可复制的内容。先点“生成提示词”，生成完成后再复制。", true);
    return;
  }
  try {
    await copyToClipboard(lastPromptText);
    setActionNote("#promptCopyStatus", "已复制。你可以把它粘贴到其它图片工具里继续用。");
    showButtonFeedback(button, "已复制");
  } catch (error) {
    setActionNote("#promptCopyStatus", error.message, true);
  }
}
...
function boot() {
  ...
  $("#runResearchBtn").addEventListener("click", () => runResearch().catch((err) => {
    showTaskError(err.message, "#researchReport");
  }));
  $("#copyResearchBtn").addEventListener("click", () => navigator.clipboard.writeText(lastResearchText || $("#researchReport").innerText));
  $("#runPromptBtn").addEventListener("click", () => runPrompt().catch((err) => {
    showTaskError(err.message, "#promptReport");
    setActionNote("#promptCopyStatus", "这次没有生成成功，所以暂时没有提示词可复制。", true);
  }));
