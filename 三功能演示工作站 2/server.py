...
def config_value(payload, key, default=""):
    config = load_config()
    return str(config.get(key) or payload.get(key) or default).strip()
        ...
        if self.path == "/api/config-status":
            ...
            self.send_json(
                200,
                {
                    ...
                    "cloudflare": bool(config.get("cloudflareAccountId") and config.get("cloudflareApiToken")),
                    "imageModel": config.get("cloudflareImageModel", ""),
                },
            )
            return
        super().do_GET()
